...
import { Check, Plus, Trash2, Pencil, RotateCcw, CheckCircle2, Loader2 } from "lucide-react";
  ...
  return (
    <div className={cn("group flex items-center gap-3 px-4 py-3 rounded-xl border border-border bg-card hover:border-border/80 transition-colors", done && "opacity-60")}>
      ...
      {editing ? (
        ...
        <span className={cn("flex-1 text-sm", done && "line-through text-muted-foreground")}>{text}</span>
      )}

      {!editing && (
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button onClick={() => setEditing(true)} className="p-1 text-muted-foreground hover:text-foreground rounded"><Pencil className="w-3.5 h-3.5" /></button>
          <button onClick={() => deleteRitual(zone, id)} className="p-1 text-muted-foreground hover:text-destructive rounded"><Trash2 className="w-3.5 h-3.5" /></button>
        </div>
      )}
    </div>
  );
...
function AddRitualRow({ zone }: { zone: RitualZone }) {
  ...
  if (open) {
    return (
      <div className="flex gap-2">
        <Input autoFocus placeholder="Nom du rituel…" value={text}
          onChange={(e) => setText(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter") submit(); if (e.key === "Escape") setOpen(false); }}
          className="h-9" />
        <Button size="sm" onClick={submit} disabled={!text.trim() || loading}>
  ...
  return (
    <button onClick={() => setOpen(true)}
      ...
      <Plus className="w-4 h-4" />Ajouter un rituel
    </button>
  );
}

export function RitualList({ zone }: { zone: RitualZone }) {
  const { rituals, loading, resetZone } = useRituelsStore();
  ...
  return (
    <div className="space-y-5">
      {list.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">{doneCount}/{list.length} accomplis</span>
            <div className="flex items-center gap-3">
              {allDone && (
                <span className="flex items-center gap-1 text-xs font-medium text-emerald-500">
                  <CheckCircle2 className="w-3.5 h-3.5" />Rituel complet !
                </span>
              )}
              <button onClick={() => resetZone(zone)}
      ...
      <div className="space-y-2">
        {list.map((r) => <RitualRow key={r.id} zone={zone} id={r.id} text={r.text} done={r.done} />)}
